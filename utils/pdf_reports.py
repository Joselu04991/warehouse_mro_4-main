...
def create_pdf_reporte(user_id):
    """
    Genera el PDF CORPORATIVO COMPLETO del perfil del usuario
    Incluye:
    - Datos personales
    - Puntaje anual (score)
    - KPIs
    ...
    try:
        ...
        c.drawImage(logo_path, width - 150, height - 85, width=120, height=45, mask="auto")
    except:
        pass
    ...
    c.drawString(170, top - 20, f"Rol: {user.role.upper()}")
    c.drawString(170, top - 40, f"Correo: {user.email}")
    ...
    c.setFillColorRGB(0, 59/255, 113/255)
    c.drawString(170, top - 145, f"PUNTAJE ANUAL: {score} pts")

    c.setFont("Helvetica", 11)
    c.setFillColor(colors.black)
    c.drawString(170, top - 165, f"Perfil completado: {perfil_completado}%")
    ...
    bar.bars[0].fillColor = colors.Color(0, 59/255, 113/255)
    ...
    if total > 0:
        ...
        pie_draw.add(pie)
        renderPDF.draw(pie_draw, c, width - 240, top - 360)
    ...
    qr.add_data(f"Usuario:{user.id}|Score:{score}|{security_code}")
    qr.make(fit=True)
    img = qr.make_image()
    img.save(qr_buf, format="PNG")
    qr_buf.seek(0)
    c.drawImage(ImageReader(qr_buf), width - 120, top - 200, width=80, height=80)
